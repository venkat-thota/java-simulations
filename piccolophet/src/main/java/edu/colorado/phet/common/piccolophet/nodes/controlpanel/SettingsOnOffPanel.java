...
import static com.aimxcel.abclearn.common.abclearncommon.resources.AbcLearnCommonResources.getString;
...
public class SettingsOnOffPanel extends PNode {
    ...
    public static final AbcLearnFont FONT = new AbcLearnFont( 20, true );
    ...
    public SettingsOnOffPanel( final List<Feature> icons ) {
        ...
        List<Feature> padded = icons.map( new F<Feature, Feature>() {
            @Override public Feature f( final Feature element ) {
                return new Feature( new PaddedNode( AbcLearnPNode.getMaxSize( all ), element.off ),
                                    new PaddedNode( AbcLearnPNode.getMaxSize( all ), element.on ), element.onProperty, element.component );
            }
        } );
        VBox box = new VBox( 4 );
        ...
        for ( final P2<Feature, Integer> e : padded.zipIndex() ) {
            ...
            if ( e._2() < padded.length() - 1 ) {
                box.addChild( new AbcLearnPPath( new Line2D.Double( 0, 0, box.getFullWidth(), 0 ) ) );
            }
        }
