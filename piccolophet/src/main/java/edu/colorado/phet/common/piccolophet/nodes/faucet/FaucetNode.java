...
import com.aimxcel.abclearn.common.abclearncommon.view.Dimension2DDouble;
...
import static edu.colorado.phet.common.piccolophet.PiccoloAbcLearnApplication.RESOURCES;
...
public class FaucetNode extends PNode {
    ...
    public FaucetNode( IUserComponent userComponent, final double maxFlowRate, final Property<Double> flowRate, ObservableProperty<Boolean> enabled, double faucetLength, double knobWidth, boolean snapToZeroWhenReleased ) {
        ...
        AbcLearnPPath pipeNode = new AbcLearnPPath( pipeRect, new TexturePaint( PIPE_IMAGE, new Rectangle2D.Double( 0, pipeRect.getY(), PIPE_IMAGE.getWidth(), PIPE_IMAGE.getHeight() ) ) );
        addChild( pipeNode );
        ...
        if ( SHOW_HANDLE_CENTER_AND_BOUNDS ) {
            addChild( new AbcLearnPPath( new Ellipse2D.Double( -2, -2, 4, 4 ), Color.PINK ) {{
                setOffset( HANDLE_CENTER );
            }} );
            addChild( new AbcLearnPPath( new Rectangle2D.Double( 0, 0, HANDLE_SIZE.getWidth(), HANDLE_SIZE.getHeight() ), new BasicStroke( 1 ), Color.GREEN ) );
        }
    ...
    public static void main( String[] args ) {
        ...
        final AbcLearnPCanvas canvas = new AbcLearnPCanvas() {{
