...
public class OutlineTextNode extends PNode {
    ...
    public OutlineTextNode( String text, Font font, Color fillColor, Color outlineColor, double outlineStrokeWidth ) {
        this.font = font;
        textPPath = new AbcLearnPPath( fillColor, new BasicStroke( (float) outlineStrokeWidth ), outlineColor );
    ...
    public static void main( String[] args ) {
        ...
        AbcLearnPCanvas canvas = new AbcLearnPCanvas();
        canvas.setPreferredSize( stageSize );
        canvas.setWorldTransformStrategy( new AbcLearnPCanvas.CenteredStage( canvas, stageSize ) );
        ...
        for ( int i = 0; i < 5; i++ ) {
            ...
            AbcLearnFont font = new AbcLearnFont( Font.PLAIN, ( i + 1 ) * smallestFontSize );
        ...
        // Arabic
        OutlineTextNode outlineTextNode5 = new OutlineTextNode( "\uFE9D\u06B0\u06AA\uFEB5", AbcLearnFont.getPreferredFont( new Locale( "ar" ), Font.PLAIN, 64 ), fillColor, strokeColor, strokeWidth );
        outlineTextNode5.setOffset( xMargin, lastNode.getFullBoundsReference().getMaxY() + ySpacing );
        canvas.addWorldChild( outlineTextNode5 );

        // Chinese
        OutlineTextNode outlineTextNode6 = new OutlineTextNode( "\u4e2d\u56fd\u8bdd\u4e0d", AbcLearnFont.getPreferredFont( new Locale( "zh" ), Font.PLAIN, 64 ), fillColor, strokeColor, strokeWidth );
