...
public class HSliderNode extends SliderNode {
    ...
    public AbcLearnPPath getTrackNode() { return node.getTrackNode();}

    //Adds a label to the slider node and return the created tick
    public AbcLearnPPath addLabel( double value, PNode labelNode ) {
        return addLabel( value, labelNode, 15, 8 );
    }

    //Adds a label to the slider node and return the created tick
    //Note: the usage of this method and esp. the last two parameters is subject to change
    public AbcLearnPPath addLabel( double value, PNode labelNode, double tickLength, double tickOffset ) {
        ...
        final AbcLearnPPath tickMark = new AbcLearnPPath( new Line2D.Double( label.getCenterX() - tickOffset, label.getCenterY(),
    ...
    public static void main( String[] args ) {
        ...
        String[] myArgs = { "-study" };
        SimSharingManager.init( new AbcLearnApplicationConfig( myArgs, "myProject" ) );
            ...
            public void run() {
                new PFrame( "test", false, new PCanvas() {{
                    ...
                    final HSliderNode sliderNode = new HSliderNode( new UserComponent( "mySlider" ), 0, 100,
                        ...
                        addLabel( 0.0, new AbcLearnPText( "None" ) );
                        addLabel( 100.0, new AbcLearnPText( "Lots" ) );
                        setOffset( 150, 250 );
                    }};
