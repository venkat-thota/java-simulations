...
public class ToggleButtonNode extends PNode {
    ...
    public ToggleButtonNode( final PNode node, final ObservableProperty<Boolean> selected, final VoidFunction0 pressed, final Color pressedInColor, final boolean disableCursorWhenPressedIn,
        ...
        final AbcLearnPPath hiddenBorder = new AbcLearnPPath( RectangleUtils.expand( node.getFullBounds(), pressAmountX, pressAmountY ), null );
        ...
        final AbcLearnPPath buttonBackground = new AbcLearnPPath( shape, DEFAULT_BACKGROUND_COLOR, new BasicStroke( 1, BasicStroke.CAP_ROUND, BasicStroke.JOIN_MITER ), null );
        final AbcLearnPPath shadow = new AbcLearnPPath( shape, Color.darkGray );
