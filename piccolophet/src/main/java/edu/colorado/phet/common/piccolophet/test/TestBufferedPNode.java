    ...
    static class ExpensiveNode extends PNode {
        ...
        public ExpensiveNode() {
            ...
            for ( int i = 0; i < 2000; i++ ) {
                AbcLearnPPath path = new AbcLearnPPath( new Line2D.Double( random.nextDouble() * w, random.nextDouble() * h, random.nextDouble() * w, random.nextDouble() * h ), new BasicStroke( random.nextFloat() * 3 ), new Color( random.nextFloat(), random.nextFloat(), random.nextFloat() ) );
                addChild( path );
            }
        }
    }
