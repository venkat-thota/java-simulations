...
public class SliderNode extends PNode {
    ...
    public SliderNode( double min, double max, double value ) {
        ...
        AbcLearnPPath backgroundForMouseEventHandling = new AbcLearnPPath( new Color( 0, 0, 0, 0 ) );
    ...
    protected class TrackNode extends PNode {
        protected TrackNode() {
            PPath path = new AbcLearnPPath( createTrackShape( min, max ), Color.lightGray, new BasicStroke( 1 ), Color.black );
            addChild( path );
        }
    }
    ...
    protected class KnobNode extends PNode {
        ...
        private KnobNode() {
            knob = new AbcLearnPPath( new RoundRectangle2D.Double( 0, 0, knobWidth, knobHeight, 6, 6 ), new Color( 237, 200, 120 ), new BasicStroke(), Color.black );
            knob.setOffset( -knob.getFullBounds().getWidth() / 2, height / 2 - knob.getFullBounds().getHeight() / 2 );
            addInputEventListener( new CursorHandler() );//todo: fails for PNode inside AbcLearnPCanvas wrapped inside JComponent inside PSwing inside AbcLearnPCanvas, see workaround below
