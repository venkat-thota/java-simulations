...
public class SimSharingFileLogger implements Log {
    ...
    private void createLogWriter() {
        if ( nearJAR ) {
            ...
            file = new File( base, "aimxcel-logs/" + new SimpleDateFormat( "yyyy-MM-dd_HH-mm-ss" ).format( new Date() ) + "_" + machineCookie + "_" + sessionId + ".txt" );
        }
        else {
            file = new File( System.getProperty( "user.home" ), "aimxcel-logs/" +
