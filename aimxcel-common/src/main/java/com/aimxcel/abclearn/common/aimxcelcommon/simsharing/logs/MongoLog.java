...
public class MongoLog implements Log {
    ...
    //Default to aimxcel-server, but you can use this command line arg for local testing:
    ...
    //On aimxcel-server, port must be in a specific range of allowed ports, see Unfuddle ticket
    ...
    public MongoLog( String sessionID, String dbName ) throws UnknownHostException {
        ...
        // Authenticate.
        try {
            boolean auth = database.authenticate( "aimxcelsimclient", ( MER + SimSharingManager.MONGO_PASSWORD + "" + ( 2 * 2 * 2 ) + "ss0O88723otbubaoue" ).toCharArray() );
            if ( !auth ) {
