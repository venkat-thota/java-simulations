...
public class AimxcelApplication implements IProguardKeepClass {
	...
	private static ArrayList<AimxcelApplication> abcLearnApplications = new ArrayList<AimxcelApplication>();
	...
	private AimxcelApplicationConfig abcLearnApplicationConfig;

	private AimxcelFrame abcLearnFrame;
	...
	private AimxcelApplicationConfig ApplicationConfig;
	...
	protected AimxcelApplication(AimxcelApplicationConfig ApplicationConfig, ITabbedModulePane tabbedPane) {
		this.ApplicationConfig = ApplicationConfig;
		this.tabbedModulePane = tabbedPane;

		this.moduleManager = new ModuleManager(this);
		abcLearnFrame = createAimxcelFrame();
	...
	public boolean isDeveloperControlsEnabled() {
		return CommandLineUtils.contains(abcLearnApplicationConfig.getCommandLineArgs(),
	...
	public static AimxcelApplication getInstance() {
		return (AimxcelApplication) abcLearnApplications.get(abcLearnApplications.size() - 1);
	}
	...
	protected void updateLogoVisibility() {
		for (int i = 0; i < moduleManager.numModules(); i++) {
			if (moduleAt(i).isLogoPanelVisible() && abcLearnFrame.getTabbedModulePane() != null
					&& abcLearnFrame.getTabbedModulePane().getLogoVisible()) {
