...
import com.aimxcel.abclearn.platetectonics.control.DensitySensorNode3D;
import com.aimxcel.abclearn.platetectonics.control.DraggableTool2D;
import com.aimxcel.abclearn.platetectonics.control.RulerNode3D;
import com.aimxcel.abclearn.platetectonics.control.ThermometerNode3D;
import com.aimxcel.abclearn.platetectonics.control.ToolDragHandler;
import com.aimxcel.abclearn.platetectonics.control.ToolboxNode;
...
import com.aimxcel.abclearn.lwjgl.CanvasTransform.StageCenteringCanvasTransform;
...
import com.aimxcel.abclearn.lwjgl.nodes.OrthoSwingNode;
...
public abstract class PlateTectonicsTab extends LWJGLTab {
    ...
    protected OrthoCoreNode draggedCrustPiece = null;
                ...
                new UpdateListener() {
                    public void update() {
                        ...
                        if ( Mouse.getEventButton() == 0 ) {
                            ThreadedPlanarCoreNode toolCollision = getToolUnder( Mouse.getEventX(), Mouse.getEventY() );
                            OrthoSwingNode guiCollision = getGuiUnder( Mouse.getEventX(), Mouse.getEventY() );

                            // if mouse is down
                            if ( Mouse.getEventButtonState() ) {
                                if ( guiCollision instanceof OrthoCoreNode && ( (OrthoCoreNode) guiCollision ).getNode() instanceof CrustPieceNode ) {
                                    draggedCrustPiece = (OrthoCoreNode) guiCollision;
...
//            glMaterial( GL_FRONT, GL_SHININESS, shininess );
    ...
    public ThreadedPlanarCoreNode getToolUnder( int x, int y ) {
        // iterate through the tools in reverse (front-to-back) order
        for ( GLNode node : getToolNodes() ) {
            ThreadedPlanarCoreNode tool = (ThreadedPlanarCoreNode) node;
    ...
    public void updateCursor() {
        ...
        final ThreadedPlanarCoreNode toolCollision = getToolUnder( Mouse.getEventX(), Mouse.getEventY() );
    ...
    public void pickedCrustPiece( OrthoCoreNode crustPiece ) {
        CrustPieceNode piece = (CrustPieceNode) crustPiece.getNode();
        // overridden in PlateMotionTab
        SimSharingManager.sendUserMessage( UserComponents.crustPiece, UserComponentTypes.sprite, com.aimxcel.abclearn.common.aimxcelcommon.simsharing.messages.UserActions.startDrag,
                                           new ParameterSet( new Parameter( ParameterKeys.plateType, piece.type.toString() ) ) );
    }

    public void movedCrustPiece( OrthoCoreNode crustPiece ) {
        // overridden in PlateMotionTab
    }

    public void droppedCrustPiece( OrthoCoreNode crustPiece ) {
        // overridden in PlateMotionTab
    }
