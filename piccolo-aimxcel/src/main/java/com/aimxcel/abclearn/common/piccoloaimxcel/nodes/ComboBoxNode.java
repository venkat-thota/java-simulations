...
import com.aimxcel.abclearn.common.piccoloaimxcel.nodes.layout.VBox;
...
public class ComboBoxNode<T> extends PNode {
    ...
    private static final Font DEFAULT_FONT = new AimxcelFont( 18 );
    ...
    private static class TriangleNode extends PNode {
        private TriangleNode() {
            addChild( new AimxcelPPath( new Rectangle2D.Double( 0, 0, 20, 20 ), Color.lightGray ) );
            addChild( new AimxcelPPath( new DoubleGeneralPath( 2, 8 ) {{
                lineToRelative( 8, 8 );
                lineToRelative( 8, -8 );
                closePath();
    ...
    private static class NodeWithBackground extends PNode {
        ...
        public NodeWithBackground( PNode node, double width ) {
            ...
            background = new AimxcelPPath( new Rectangle2D.Double( -2, 0, width + 2 + 2, node.getFullBounds().getHeight() ), plainColor );
