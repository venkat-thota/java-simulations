...
import com.aimxcel.abclearn.common.abclearncommon.updates.IVersionSkipper;
import com.aimxcel.abclearn.common.abclearncommon.updates.SimUpdater;
...
public abstract class SimAbstractUpdateDialog extends PaintImmediateDialog {
    ...
    private static final String SKIP_UPDATE_BUTTON = AbcLearnCommonResources.getString( "Common.updates.skipThisUpdate" );
    private static final String CANCEL_BUTTON = AbcLearnCommonResources.getString( "Common.choice.cancel" );
    private static final String TRY_IT_LINK = AbcLearnCommonResources.getString( "Common.updates.tryIt" );

    private final ISimInfo simInfo;
    private final AbcLearnVersion newVersion;

    protected SimAbstractUpdateDialog( Frame owner, ISimInfo simInfo, AbcLearnVersion newVersion ) {
        super( owner, TITLE );
        setModal( true );
        setResizable( false );
    ...
    */
     ...
     */
    private JPanel createMessagePanel( ISimInfo simInfo, AbcLearnVersion newVersion ) {

        final String project = simInfo.getProjectName();
        final String sim = simInfo.getFlavor();
        String simName = simInfo.getName();
        AbcLearnVersion currentVersion = simInfo.getVersion();
        ...
        tryItLink.addMouseListener( new MouseAdapter() {
            public void mousePressed( MouseEvent e ) {
                AbcLearnServiceManager.showSimPage( project, sim );
            }
        } );
    ...
    */
    protected abstract JPanel createButtonPanel( ISimInfo simInfo, AbcLearnVersion newVersion );
    ...
    */
    private static String getVersionComparisonHTML( String simName, String currentVersion, String newVersion ) {
        String pattern = AbcLearnCommonResources.getString( "Common.updates.versionComparison" );
        Object[] args = { simName, currentVersion, newVersion };
        return MessageFormat.format( pattern, args );
    }

    protected static class UpdateButton extends JButton {
        public UpdateButton( final JDialog dialog, final ISimInfo simInfo, final AbcLearnVersion newVersion ) {
            super( AbcLearnCommonResources.getString( "Common.updates.updateNow" ) );
            addActionListener( new ActionListener() {
                public void actionPerformed( ActionEvent e ) {
                    ...
                    if ( DeploymentScenario.getInstance().isUpdatable() ) {
                        ...
                        AbcLearnServiceManager.showSimPage( simInfo.getProjectName(), simInfo.getFlavor() );
                    }
                }
            } );
        }
    ...
    protected static class SkipVersionButton extends JButton {
        public SkipVersionButton( final JDialog dialog, final IVersionSkipper versionSkipper, final AbcLearnVersion newVersion ) {
