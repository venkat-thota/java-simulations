...
import com.aimxcel.abclearn.common.abclearncommon.util.AbcLearnLocales;
 ...
 */
public class AbcLearnLocales implements Serializable {
    ...
    private static AbcLearnLocales _singleton;

    private HashMap<Locale, String> _localeToNameMap;
    private HashMap<String, Locale> _nameToLocaleMap;

    /* singleton */
    public static AbcLearnLocales getInstance() {
        if ( _singleton == null ) {
            _singleton = new AbcLearnLocales();
        }
        return _singleton;
    }

    /* singleton */
    private AbcLearnLocales() {
        _nameToLocaleMap = new HashMap<String, Locale>();
        _localeToNameMap = new HashMap<Locale, String>();
        loadCodes( AbcLearnCommonResources.getInstance().getProperties( PROPERTIES_RESOURCE_NAME ) );
    }
