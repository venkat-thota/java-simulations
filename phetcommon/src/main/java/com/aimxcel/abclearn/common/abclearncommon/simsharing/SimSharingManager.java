...
import static com.aimxcel.abclearn.common.abclearncommon.simsharing.SimSharingMessage.MessageType.model;
import static com.aimxcel.abclearn.common.abclearncommon.simsharing.SimSharingMessage.MessageType.system;
import static com.aimxcel.abclearn.common.abclearncommon.simsharing.SimSharingMessage.MessageType.user;
...
import static com.aimxcel.abclearn.common.abclearncommon.simsharing.messages.SystemActions.sentEvent;
import static com.aimxcel.abclearn.common.abclearncommon.simsharing.messages.SystemActions.started;
...
import com.aimxcel.abclearn.common.abclearncommon.simsharing.components.SimSharingIdDialog;
import com.aimxcel.abclearn.common.abclearncommon.simsharing.logs.ConsoleLog;
import com.aimxcel.abclearn.common.abclearncommon.simsharing.logs.MongoLog;
import com.aimxcel.abclearn.common.abclearncommon.simsharing.logs.StringLog;
...
import com.aimxcel.abclearn.common.abclearncommon.simsharing.messages.ModelMessage;
...
import com.aimxcel.abclearn.common.abclearncommon.simsharing.messages.SystemMessage;
...
import com.aimxcel.abclearn.common.abclearncommon.simsharing.messages.UserMessage;
import com.aimxcel.abclearn.common.abclearncommon.simsharing.tests.MongoLoadTesterSimLauncher;
...
import com.aimxcel.abclearn.common.abclearncommon.simsharing.SimSharingConfig;
import com.aimxcel.abclearn.common.abclearncommon.simsharing.SimSharingFileLogger;
...
import com.aimxcel.abclearn.common.abclearncommon.simsharing.SimSharingPropertiesFile;
...
public class SimSharingManager {
    ...
    public static final SimSharingManager getInstance() {
        ...
        if ( INSTANCE == null ) {
            init( new AbcLearnApplicationConfig( new String[]{}, "sim-sharing-manager" ) );
        }
        assert ( INSTANCE != null ); // make sure init worked
        return INSTANCE;
    }

    // Use if you have a non-AbcLearnApplication (e.g., a sample main) that involves sim-sharing and you want basic console output.
    public static void init() {
        init( new AbcLearnApplicationConfig( new String[]{"-study"}, "sim-sharing-manager" ) );
    }

    // Initialization, creates the singleton and sends startup events if sim-sharing is enabled.
    public static void init( final AbcLearnApplicationConfig config ) {
        init( config, SIM_EVENT_COLLECTION_DB_NAME );
    }

    public static void init( final AbcLearnApplicationConfig config, String dbName ) {
    ...
    private SimSharingManager( final AbcLearnApplicationConfig applicationConfig, String dbName ) {
     ...
     * Calling this method before AbcLearnApplicationLauncher.launchSim is a programming error, and will
     * result in IllegalStateException. We can't test for a SimSharingConfig until we receive the
     * AbcLearnApplicationConfig from AbcLearnApplicationLauncher, which contains the name of the study.
     * And we don't call getInstance herein, because getInstance will happily initialize with a
     * dummy AbcLearnApplicationConfig.
     */
    ...
    */
    ...
    private void sendStartupMessage( AbcLearnApplicationConfig config ) {
        assert ( enabled );
